...
class TestBaiDu:
    ...
    @pytest.mark.parametrize('content,assertion,assertype', reda_web_casedata(yamlfile, 'test_baidu_search'))  # 测试数据
    def test_baidu_search(self, webDriver, content,assertion,assertype):

        with allure.step('输入搜索内容'):
            baidu = AutoRunCase(webDriver)

            baidu.run(yamlfile,'test_baidu_search',test_date=[content], assertion=assertion, assertype=assertype)
